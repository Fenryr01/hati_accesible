...
$sql_persona = "INSERT INTO personas (nombre, apellido, dni, nacimiento, edad, contacto, domicilio, barrio, tipo_tenencia, procedencia_agua, cantidad_camas, ventilacion, iluminacion, higiene, orden, existencia_sanitaria, letrina, barreras_arquitectonicas, cobertura, cud, lugar_atencion, necesita_asistencia, quien_brinda_asistencia, cobra_pension)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";

$stmt_persona = $conexion->prepare($sql_persona);
$stmt_persona->bind_param("ssssssssssssssssssssssss", $nombre, $apellido, $dni, $nacimiento, $edad, $contacto, $domicilio, $barrio, $tipo_tenencia, $procedencia_agua, $cantidad_camas, $ventilacion, $iluminacion, $higiene, $orden, $existencia_sanitaria, $letrina, $barreras_arquitectonicas, $cobertura, $cud, $lugar_atencion, $necesita_asistencia, $quien_brinda_asistencia, $cobra_pension);
...
if ($stmt_persona->execute()) {
    ...
    $sql_grupo_familiar = "INSERT INTO grupo_familiar (persona_id, quien, edad, escolaridad, trabajo, donde) VALUES (?, ?, ?, ?, ?, ?)";
    ...
    for ($i = 1; $i <= $miembros_grupo_familiar; $i++) {
        ...
        $stmt_grupo_familiar->bind_param("isssss", $persona_id, $quien, $edad, $escolaridad, $trabajo, $donde);
    ...
    if (isset($_POST['discapacidad']) && is_array($_POST['discapacidad']) && !empty($_POST['discapacidad'])) {
        $sql_discapacidad = "INSERT INTO discapacidades (persona_id, tipo_discapacidad) VALUES (?, ?)";
        ...
        foreach ($_POST['discapacidad'] as $tipo) {
            if (!empty($tipo)) { // Verificar que el valor no esté vacío
                $stmt_discapacidad->bind_param("is", $persona_id, $tipo);
